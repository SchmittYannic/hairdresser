...
const MultiRangeSlider = <T extends string | number>({
    ...
    return (
        <div className="multi-slider-container">
            ...
            <div className="slider">
                ...
                <div className="slider__left-value">{displayMin}</div>
                <div className="slider__right-value">{displayMax}</div>
            </div>
        </div>
    )
    ...
    // const getPercent = useCallback(
    //     (value: any) => Math.round(((value - min) / (max - min)) * 100), [min, max]
    // );
    ...
    //     <>
    //         <input
    //             className={`thumb${minVal > max - 100 ? " thumb--zindex-5" : " thumb--zindex-3"}`}
    //             type="range"
    //             min={min}
    //             max={max}
    //             step={step}
